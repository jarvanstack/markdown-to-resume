...
const githubStyleContract = {
  ...
  h1: { selector: 'h1', properties: ['font-size', 'line-height', 'font-weight', 'color', 'margin-top', 'border-bottom-width', 'border-bottom-style', 'border-bottom-color'] },
  h2: { selector: 'h2', properties: ['font-size', 'line-height', 'font-weight', 'color', 'margin-top', 'border-bottom-width', 'border-bottom-style', 'border-bottom-color'] },
  h3: { selector: 'h3', properties: ['font-size', 'line-height', 'font-weight', 'color'] },
  h4: { selector: 'h4', properties: ['font-size', 'line-height', 'font-weight', 'color'] },
  ...
  paragraph: { selector: ':scope > p', properties: ['font-size', 'line-height', 'color'] },
  ...
  blockquote: { selector: 'blockquote', properties: ['color', 'margin-right', 'margin-left', 'padding-right', 'padding-left', 'border-left-width', 'border-left-style', 'border-left-color'] },
  blockquoteParagraph: { selector: 'blockquote > p', properties: ['margin-top', 'margin-bottom'] },
  orderedList: { selector: ':scope > ol', properties: ['padding-left', 'list-style-type'] },
  ...
  unorderedList: { selector: ':scope > ul:not(.contains-task-list)', properties: ['padding-left', 'list-style-type'] },
  adjacentUnorderedItem: { selector: ':scope > ul:not(.contains-task-list) > li + li', properties: ['margin-top'] },
  table: { selector: 'table', properties: ['display', 'max-width', 'overflow-x', 'border-collapse', 'font-variant-numeric'] },
  ...
  fencedCode: { selector: ':scope > pre', properties: ['font-family', 'font-size', 'line-height', 'color', 'background-color', 'padding-top', 'padding-right', 'padding-bottom', 'padding-left', 'overflow-x', 'border-radius'] },
  ...
  taskList: { selector: 'ul.contains-task-list', properties: ['padding-left'] },
  ...
  horizontalRule: { selector: 'hr', properties: ['box-sizing', 'height', 'overflow', 'padding-top', 'padding-right', 'padding-bottom', 'padding-left', 'background-color', 'border-top-width', 'border-top-style'] },
} as const;
...
test('按系统语言初始化并持久化用户的语言选择', async ({ browser }) => {
  ...
  try {
    await page.goto('/');
    await expect(page).toHaveTitle('Markdown Resume Editor - Smart One-Page PDF Builder | Moli');
    ...
    await expect(page).toHaveTitle('Markdown 简历编辑器 - 智能一页排版与免费 PDF 导出 | 墨历');
...
test('CSS 页按主题编辑完整样式并实时预览、持久化和导出', async ({ page }) => {
  ...
  await expect(page.locator('[aria-label="CSS 编辑器"]')).toBeVisible();

  const cssEditor = page.locator('.source-editor-panel .cm-content');
  await expect(cssEditor).toContainText('.theme.github');
  const originalHeaderColor = await page.locator('.panel-header').first().evaluate((element) => getComputedStyle(element).color);
  await cssEditor.click();
  await page.keyboard.press(process.platform === 'darwin' ? 'Meta+A' : 'Control+A');
  await page.keyboard.insertText('.theme.github h1 { color: rgb(192, 38, 61) !important; }\n.panel-header { color: rgb(255, 0, 0) !important; }');
  await expect.poll(() => page.getByTestId('resume-page').locator('h1').evaluate((element) => getComputedStyle(element).color)).toBe('rgb(192, 38, 61)');
  await expect.poll(() => page.locator('.panel-header').first().evaluate((element) => getComputedStyle(element).color)).toBe(originalHeaderColor);

  await page.getByTestId('theme-crisp').click();
  await expect(cssEditor).toContainText('.theme.crisp');
  await expect(cssEditor).not.toContainText('rgb(192, 38, 61)');
  await page.getByTestId('theme-github').click();
  await expect(cssEditor).toContainText('rgb(192, 38, 61)');
  await expect.poll(() => page.evaluate(() => localStorage.getItem('markdown-resume-theme-css-v1'))).toContain('rgb(192, 38, 61)');
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem('markdown-resume-state-v3') ?? '{}').settings?.theme)).toBe('github');

  await page.reload();
  await page.getByRole('tab', { name: 'CSS', exact: true }).click();
  await expect(page.locator('.source-editor-panel .cm-content')).toContainText('rgb(192, 38, 61)');
  ...
  expect(html).toContain('.theme.css-editor-theme h1');
  expect(html).toContain('rgb(192, 38, 61)');

  await page.getByRole('button', { name: '恢复当前主题 CSS' }).click();
  await expect(page.locator('.source-editor-panel .cm-content')).toContainText('.theme.github');
  await expect(page.locator('.source-editor-panel .cm-content')).not.toContainText('rgb(192, 38, 61)');
});
...
test('主题二级页提供全部 11 个主题、可以滚动并可载入编辑器', async ({ page }) => {
  ...
  await expect(page.getByTestId('resume-page')).toHaveAttribute('data-theme-id', 'fresh');
});
...
test('GitHub 主题计算样式与 github-markdown-css 5.9.0 一致', async ({ page, context }) => {
  ...
  await expect(resume).toHaveClass(/css-editor-theme/);
  await expect(resume).toHaveAttribute('data-theme-id', 'github');
...
test('导出有效的中文 PNG 和 PDF', async ({ page }) => {
  ...
  expect(htmlContent).toContain('.theme.css-editor-theme');
...
test('生成桌面编辑器、主题页、模板页和移动端截图', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: 'CSS', exact: true }).click();
  await expect(page.locator('[aria-label="CSS 编辑器"]')).toBeVisible();
  await page.screenshot({ path: path.join(artifactDir, 'desktop-css-editor.png'), fullPage: true });
  await page.goto('/themes');
  ...
  await page.getByRole('tab', { name: 'CSS', exact: true }).click();
  await expect(page.locator('[aria-label="CSS 编辑器"]')).toBeVisible();
  await page.screenshot({ path: path.join(artifactDir, 'mobile-css-editor.png'), fullPage: true });
